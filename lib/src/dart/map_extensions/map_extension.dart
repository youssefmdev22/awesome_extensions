...
extension MapExtension<K, V> on Map<K, V> {
  ...
  Map<K, V> filterKeys(bool Function(K) predicate) =>
      .fromEntries(entries.where((e) => predicate(e.key)));
  ...
  Map<K, V> filterValues(bool Function(V) predicate) =>
      .fromEntries(entries.where((e) => predicate(e.value)));
  ...
  Map<V, K> swap() => .fromEntries(entries.map((e) => .new(e.value, e.key)));
  ...
  MapEntry<K, V>? get randomEntry {
    ...
    DateTime now = .now();
    final randomIndex = now.millisecond % length;
